...
# 
# SL_lib <- c("SL.gam", "SL.earth", "SL.glm")
...
tmle <- function(data, parametric) {
  V <- ifelse(nrow(data) < 1000, 10L, 2L)
  stack <- {
    if (parametric) 
      sl3::make_learner_stack(sl3::Lrnr_glm_fast)
    else
      sl3::make_learner_stack(sl3::Lrnr_glm_fast, sl3::Lrnr_ranger, sl3::Lrnr_xgboost)
  }
  lmtp::lmtp_tmle(as.data.frame(data), "trt", "outcome", grep("^cnf", names(data), value = TRUE), 
                  outcome_type = "binomial", 
                  shift = static_binary_on, folds = V, .SL_folds = V,
                  learners_outcome = stack, 
                  learners_trt = stack)$theta
}
